...
export default function ClientRagnarConfigPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchConfig = async () => {
        try {
            const res = await fetch(`/api/clients/${clientId}/ragnar`);
            ...
            setMessage({ type: 'error', text: 'Falha ao carregar configuração.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading) return <div className="p-8 text-slate-400 animate-pulse">Carregando Ragnar...</div>;
    ...
    return (
        <div className="font-sans min-h-screen pb-20 relative">
            ...
            <header className="mb-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
                <div>
                     <button onClick={() => router.push('/dashboard/clients')} className="text-slate-500 hover:text-white transition-colors text-sm mb-4 flex items-center gap-2">
                        <span className="text-lg">←</span> Voltar para Clientes
                    </button>
                    <h1 className="text-4xl font-black mb-2 text-white tracking-tight">Ragnar (Cliente)</h1>
                    <p className="text-slate-400 text-lg">Configure as instruções de auditoria específicas para este cliente.</p>
                </div>
