...
export default function SessionView({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="h-[calc(100vh-2rem)] flex flex-col font-sans animate-fade-in-up">
            {/* Header */}
            {/* Header */}
            <header className={`mb-6 flex flex-col md:flex-row justify-between md:items-center bg-slate-800/20 backdrop-blur-xl p-4 md:p-5 rounded-[2rem] border transition-all duration-700 shadow-2xl gap-4 md:gap-0 ${session.simulationMode === 'LABORATORY' ? 'border-emerald-500/20 bg-emerald-950/10' : 'border-slate-700/30'}`}>
                <div className="flex items-center gap-3 md:gap-5">
                    <div className="relative group shrink-0">
                        <div className={`w-3 h-3 md:w-4 md:h-4 rounded-full ${session.status === 'RUNNING' ? (session.simulationMode === 'LABORATORY' ? 'bg-emerald-500 shadow-[0_0_15px_#10b981]' : 'bg-blue-500 shadow-[0_0_15px_#3b82f6]') : 'bg-slate-600'}`}></div>
                    ...
                    <div>
                        <div className="flex items-center gap-2 md:gap-3 flex-wrap">
                            <h1 className="text-lg md:text-xl font-black text-white flex items-center gap-2 md:gap-3 tracking-tight">
                                Sessão <span className="text-slate-500 font-mono text-xs md:text-sm font-medium tracking-normal opacity-50">#{session.id.slice(0, 8)}</span>
                            </h1>
                            <span className={`text-[8px] md:text-[9px] font-black px-2 md:px-3 py-0.5 md:py-1 rounded-full uppercase tracking-widest border ${session.simulationMode === 'LABORATORY' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'}`}>
                                {session.simulationMode === 'LABORATORY' ? 'LAB' : 'WHATSAPP'} 
                            </span>
                        </div>
                        <div className="flex items-center gap-2 md:gap-3 mt-1 flex-wrap">
                            <p className="text-[10px] md:text-xs font-bold text-slate-400 truncate max-w-[150px] md:max-w-none">
                                {session.agentVersion?.agent?.name || 'Agente'} • <span className="text-slate-500 font-medium">v{session.agentVersion?.versionNumber}</span>
                            </p>
                            <span className="w-1 h-1 rounded-full bg-slate-700"></span>
                            <span className={`text-[10px] md:text-xs font-bold uppercase tracking-widest ${session.status === 'RUNNING' ? 'text-indigo-400' : 'text-slate-500'}`}>{session.status}</span>
                        </div>
                    </div>
                </div>
                <div className="flex gap-2 md:gap-4 self-end md:self-auto w-full md:w-auto justify-end">
                    {session.status === 'RUNNING' ? (
                        <button
                            onClick={handleStop}
                            className="bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 px-4 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-2xl transition-all text-[10px] md:text-xs font-black uppercase tracking-widest active:scale-95 flex-1 md:flex-none justify-center flex"
                        >
                            Parar
                        </button>
                    ) : (
                        <button
                            ...
                            className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-4 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-2xl transition-all text-[10px] md:text-xs font-black uppercase tracking-widest active:scale-95 flex-1 md:flex-none justify-center flex"
                        >
                            Retomar
                        </button>
                    )}
                    <button 
                        ...
                        className="text-slate-400 hover:text-white px-4 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-2xl border border-slate-700/50 hover:bg-slate-800 transition-all text-[10px] md:text-xs font-black uppercase tracking-widest active:scale-95"
                    >
                        Sair
                    </button>
                </div>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-8 min-h-0">
                ...
                <div className="lg:col-span-3 glass-panel rounded-[2.5rem] border-slate-800/50 flex flex-col overflow-hidden relative shadow-2xl">
                    {/* Messages Area */}
                    <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 relative z-10 scroll-smooth flex flex-col" ref={scrollRef}>
